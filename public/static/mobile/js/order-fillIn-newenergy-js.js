...
$(document).ready(function () {
    ...
    var goods_id = getValueByParameter("goods_id");
    var order_id = "";
    getDriver(driverId);
    //获取预定信息,做界面初始化
    //记录上一个页面(或者返回时到达的页面)
    ...
    function get_car_info(goods_id) {
        $.ajax({
            url: website + "/api/Car/get_car_id.html",
            type: "POST",
            async: true,
            data: {
                id: goods_id
            ...
            success: function (data_temp) {
                ...
                if (parseInt(data_temp.code) === 0) {
                    ...
                    $(".js_car_plate").attr("data-navi-lat", data.location_latitude);
                    $(".js_car_plate").attr("data-navi-lng", data.location_longitude);
    ...
    function add_reserve_handle(data, car_id) {
        if (data.code === 0) {
            ...
            Toast(data.info, 2000);
            window.location.href = 'map_choose_car.html';
    ...
    function cancel_reserve_handle(data) {
        mui.alert("每天取消预订次数不得大于4次，当前剩余次数" + (4 - data.data), "温馨提示", "确认", function () {
            if (parseInt(data.code) === 0) {
                clearInterval(timer_wait);
                // location.href = "map_choose_car.html";
            }
            if (getReferrUrl() === "map_choose_car.html") {
                location.href = "map_choose_car.html";
            } else if (getReferrUrl() === "carsearch_newenergy.html") {
    ...
    function get_reserve_handle(data, car_id) {
        if (data.code === 0) {
            ...
            wait_timer(data_temp.surplus_time);
            goods_id = data_temp.goods_id;
    ...
    function getPicLastOrder(goods_id) {
        $.ajax({
            url: "http://www.youchedongli.cn/api/order/get_car_order_img.html",
            type: "GET",
            async: true,
            data: {
                goods_id: goods_id
