...
var history_url = getReferrUrl();//获取界面是从什么地方跳转过来的
    ...
    $.ajax({
        ...
        success: function (data) {
            if (parseInt(data.code) == 0) {
                ...
                $("#js_place_return_car").text(addr.substring(addr.indexOf("市") + 1,addr.length) + pois[0].name);
            }
        }
    });
});

$("#js_place_return_car").on("click",function () {

    mui.prompt("输入详细地址作为您的还车点","输入取车的详细地址","提示",["确认","取消"],function (e) {
        if(e.index == 0){
            $("#js_place_return_car").text(e.value);
        }
    },"div");
...
$("#js_choose_this_point").on("click",function () {
    ...
    setTimeout(function (){
        pt = map.getBounds().getCenter();
        var point_return = {"lat":pt.lat,"lng":pt.lng,"name":$("#js_place_return_car").text()};
        localStorage.setItem("point_return",JSON.stringify(point_return));
