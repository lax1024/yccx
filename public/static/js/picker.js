...
layui.define(['laytpl', 'form'], function (exports) {
    ...
    var Picker = function () {
        ...
        this.config = {
            ...
            url: "/api/Area/get_province_list.html",                          //远程地址，未用到
            type: 'GET'                              //请求方式,未用到
        };
        this.v = '1.0.0';
        //渲染数据
    ...
    Picker.fn.render = function () {
        ...
        var config = that.config,
            ...
            getDatas = function (type, parentCode, selectCode) {
                ...
                for (var i = 0; i < data.length; i++) {
                    var e = data[i];
                    // if (e.type == type) {
                    var isSelected = selectCode == e.id;
                    result.push({
                        code: e.id,
                        name: e.name,
            ...
            renderData = function (type, $picker, parentCode, selectCode, init) {
                ...
                switch (type) {
                    ...
                    case pickerType.area:
                        ...
                        break;
                    case pickerType.street:
                        tempHtml = temp(pickerFilter.street, STREET_TIPS);
                        filter = pickerFilter.street;
                        break;
                }
        ...
        if (config.codeConfig.default) {
            ...
            var pCode = config.codeConfig.province;
            var cCode = config.codeConfig.city;
            var aCode = config.codeConfig.area;
            var sCode = config.codeConfig.street;
            switch (pType) {
                case pickerType.province:
                    //渲染省
                    renderData(pickerType.province, $picker, null, pCode);
                    break;
                case pickerType.city:
                    //渲染省
                    renderData(pickerType.province, $picker, null, pCode);
                    //渲染市
                    renderData(pickerType.city, $picker, pCode, cCode);
                    break;
                case pickerType.area:
                    //渲染省
                    renderData(pickerType.province, $picker, null, pCode);
                    ...
                    renderData(pickerType.city, $picker, pCode, cCode);
                    //渲染区/县
                    renderData(pickerType.area, $picker, cCode, aCode);
                    break;
                case pickerType.street:
                    //渲染省
                    renderData(pickerType.province, $picker, null, pCode);
                    ...
                    renderData(pickerType.area, $picker, cCode, aCode);
                    //渲染街道
                    renderData(pickerType.street, $picker, aCode, sCode);
                    break;
            }
        } else {
