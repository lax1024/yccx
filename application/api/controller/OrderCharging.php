 ...
 */
class OrderCharging extends UserBase
{
    private $order_model;
...
//            'lease_data' => $lease_data['lease_data'],
    ...
    {
        //众泰车型可还车站点
        $zt_store = [153, 166, 167, 174, 182, 183, 184];
        $out_data['code'] = 1;
        ...
        $redis = new Redis();
        if (!$redis->has($car_data['device_number'])) {
            $out_data['code'] = 101;
            $out_data['info'] = "车机已掉线";
            return $out_data;
        } else {
            $out_data_device = $redis->get($car_data['device_number'] . "_status");
        ...
        if (!empty($car_data['series_id'])) {
            ...
            if ($series_id == 4664 && $store_pid != 307) {
                $out_data['code'] = 1020;
                $out_data['info'] = '众泰车型仅限于花溪大学城区域内还车';
                out_json_data($out_data);
            }
