 ...
 */
class Seller extends SellerAdminBase
{
    ...
    public function index($keyword = '', $page = 1)
    {
        ...
        foreach ($seller_list as $value) {
            $seller_group = $this->sellergroup_model->find($value['seller_group_id']);
            $value['seller_group_str'] = $seller_group['title'];
        }
    ...
    public function save()
    {
        if ($this->request->isPost()) {
            ...
            $in_seller_data = array(
                ...
                'parent_id' => $this->seller_info['seller_id'],
    ...
    {
        if ($this->seller_model->addLockSeller($id, false, $this->web_info['store_key_id'])) {
            $this->success('锁定成功');
        } else {
            $this->error('锁定失败');
        }
    ...
    {
        if ($this->seller_model->delLockSeller($id, false, $this->web_info['store_key_id'])) {
            $this->success('解除锁定成功');
        } else {
            $this->error('解除锁定失败');
        }
