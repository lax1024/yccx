 ...
 */
class Car extends SellerAdminBase
{
     ...
     * @return mixed
     */
    public function index($keyword = '', $page = 1, $car_status = 0)
    {
        $map = [];
        if ($keyword) {
            $map['store_key_id|brand_name|series_name|cartype_name|licence_plate|store_site_id|store_site_name'] = ['like', "%{$keyword}%"];
        }
        if (!empty($car_status)) {
            $map['car_status'] = $car_status;
        }
        $map['goods_type'] = GoodsType::$GoodsTypeCar['code'];
        $maps['store_type'] = GoodsType::$GoodsTypeCar['code'];
        $maps['store_key_id'] = $this->web_info['store_key_id'];
        $page_config = ['page' => $page, 'query' => ['car_status' => $car_status, 'keyword' => $keyword]];
        ...
        return $this->fetch('index', ['carcommon_list' => $carcommon_list, 'keyword' => $keyword, 'store_site_list' => $store_site_list, 'car_status' => $car_status, 'car_status_list' => $car_status_list]);
    }
...
//            * cartype_name 车型名称
    ...
    public function edit($id)
    {
        ...
        if ($carcommon_data['code'] == 0) {
            ...
            $car_color = CarColor::$CARCOLOR_CODE;
            $car_grade = CarGrade::$CARGRADE_CODE;
            return $this->fetch('edit', ['carcommon' => $carcommon_data['data'], 'car_grade' => $car_grade, 'brand_config' => json_encode($carcommon_data['brand_config']), 'store_site_list' => $store_site_list, 'year_list' => $year_list, 'car_color' => $car_color, 'car_status' => $car_status]);
        } else {
    ...
    public function expand($id)
    {
        ...
        if ($carcommon_data['code'] == 0) {
            ...
            $car_status = CarStatus::$CARSTATUS_CODE;
            $car_grade = CarGrade::$CARGRADE_CODE;
            return $this->fetch('edit', ['carcommon' => $carcommon_data['data'], 'car_grade' => $car_grade, 'brand_config' => json_encode($carcommon_data['brand_config']), 'store_site_list' => $store_site_list, 'year_list' => $year_list, 'car_status' => $car_status]);
        } else {
            $this->error($carcommon_data['info']);
        }
    }
