 ...
 */
class Charging extends SellerAdminBase
{
    ...
    public function index($keyword = '', $page = 1, $charging_status = 0, $charging_type = 0, $pid = 0)
    {
        ...
        $page_config = ['page' => $page, 'query' => ['charging_status' => $charging_status, 'charging_type' => $charging_type, 'pid' => $pid, 'keyword' => $keyword]];
        $charging_list = $this->charging_model->getChargingList($map, 'id DESC', 15, true, '', $page_config);
        return $this->fetch('index', ['charging_list' => $charging_list, 'keyword' => $keyword, 'charging_status' => $charging_status, 'charging_type' => $charging_type, 'pid' => $pid, 'charging_status_list' => $charging_status_list]);
    }
    ...
    {
        $map['store_key_id'] = $this->web_info['store_key_id'];
        $store_list = $this->store_model->getChildList($map, "id,store_pid,store_name");
        $charging_status = ChargingStatus::$CHARGINGSTATUS_CODE;
        $charging_type = ChargingType::$CHARGINGTYPE_CODE;
        return $this->fetch('add', ['store_list' => $store_list, 'id' => $id, 'charging_status' => $charging_status, 'charging_type' => $charging_type]);
    }
...
//            * store_key_name 电桩归属总店名称
            ...
            $out_store = $this->store_model->getStoreField($store_id, 'store_name,store_longitude,store_latitude');
            $store_info = array();
            if (!empty($out_store)) {
                ...
                $store_info = array(
                    'store_id' => 0,
                    'store_longitude' => '106.62215718',
                    'store_latitude' => '26.40585615',
                    'store_name' => "优车出行"
                );
            }
            ...
            $in_charging_data = array(
                ...
                'location_longitude' => $store_info['store_longitude'],
                'location_latitude' => $store_info['store_latitude'],
                'charging_status' => $data['charging_status'],
                'charging_type' => $data['charging_type'],
                'store_key_id' => $this->web_info['store_key_id'],
                'store_key_name' => $this->web_info['store_key_name'],
            );
