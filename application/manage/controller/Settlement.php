...
class Settlement extends AdminBase
{
    ...
    public function index($store_key_id = 0, $end_time = '', $is_all = 0, $page = 1)
    {
        ...
        $map['payment_time'] = array('between', '1483200000,' . $end_time);
        $page_config = ['page' => $page, 'query' => ['store_key_id' => $store_key_id, 'end_time' => $end_time_old, 'is_all' => $is_all]];
        ...
        if (intval($is_all) == 1) {
            ...
            $order_amount_sett = $order_amount_sett - $refund_amount;
        } else {
            $map['is_refund'] = 2;
            $refund_amount = $this->order_model->where($map)->sum('refund_amount');
            $order_amount_sett = $order_amount - $refund_amount;
        }
        if (!empty($store_data)) {
        ...
        return $this->fetch('index', ['order_list' => $order_list, 'store' => $store_data, 'is_all' => $is_all, 'count' => $count, 'order_amount' => $order_amount, 'store_key_id' => $store_key_id, 'end_time' => date('Y-m-d H:i:s', $end_time)]);
    }
