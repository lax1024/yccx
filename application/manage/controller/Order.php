...
class Order extends AdminBase
{
    ...
    public function index($keyword = '', $page = 1, $order_status = '', $start_time = '', $end_time = '', $payment_code = '', $limit = 15, $is_excl = 0)
    {
        ...
        $order_list = $this->order_model->getPageList($map, ' id DESC ', $page_config, $limit, $this->web_info['is_super_admin'], $this->web_info['store_key_id'], true);
...
//     * @param $channel_uid 渠道信息
     ...
     */
    public function refund($pay_sn, $refund_amount = 0)
    {
        if (empty($refund_amount)) {
            $this->error("退款申请失败，金额不对");
        }
//        $out_data = $this->order_model->getOrder($id, '', $this->web_info['is_super_admin'], $this->web_info['store_key_id']);
        ...
        $this->order_model->where(['pay_sn' => $pay_sn])->setField(['refund_amount' => $refund_amount]);
        $refund_sn = str_replace('pay', 'refund', $pay_sn);
        if (floatval($order_data['pay_amount']) < floatval($refund_amount)) {
            $pd_amount = floatval($refund_amount) - floatval($order_data['pay_amount']);
            ...
            $refund_amount = $order_data['pay_amount'];
        }
        //退款订单
        $input = new WxPayRefund();
        $input->setOutTradeNo($pay_sn);
        $input->setOutRefundNo($refund_sn);
        $input->setTotalFee(intval($order_data['pay_amount'] * 100));
        $input->setRefundFee(intval($refund_amount * 100));
    ...
    {
        $operation_per_model = new CarOperationPerModel();
