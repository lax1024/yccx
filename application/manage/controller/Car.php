...
class Car extends AdminBase
{
     ...
     * @return mixed
     */
    public function index($keyword = '', $page = 1, $car_status = 1)
    {
        $map = [];
        if ($keyword) {
            $map['store_key_id|brand_name|series_name|cartype_name|licence_plate|store_site_id'] = ['like', "%{$keyword}%"];
        }
        if (!empty($car_status)) {
        ...
        return $this->fetch('index', ['carcommon_list' => $carcommon_list, 'keyword' => $keyword, 'car_status' => $car_status, 'car_status_list' => $car_status_list,'store_site_list'=>$store_site_list]);
    }
...
//            * cartype_name 车型名称
    ...
    public function dispatch($id)
    {
        $carcommon_data = $this->carcommon_model->getCarCommon($id, true);
        if ($carcommon_data['code'] == 0) {
            return $this->fetch('dispatch', ['carcommon' => $carcommon_data['data']]);
        } else {
            $this->error($carcommon_data['info']);
        }
    }
