...
class AdminUser extends AdminBase
{
    ...
    public function index()
    {
        ...
        $group_list = $this->auth_group_model->where(['status' => 1])->select();
        $group_list_arr = [];
        foreach ($group_list as $value){
            $group_list_arr[intval($value['id'])] = $value['title'];
        }
        $group_access_list = $this->auth_group_access_model->where([])->select();
        $group_access_list_arr = [];
        foreach ($group_access_list as $value){
            $group_access_list_arr[intval($value['uid'])] = $value['group_id'];
        }
        foreach ($admin_user_list as &$value) {
            $value['group_name'] = $group_list_arr[$group_access_list_arr[intval($value['id'])]];
        }
        ...
        if ($this->request->isPost()) {
            ...
            if ($validate_result !== true) {
                ...
                if (!empty($data['password']) && !empty($data['confirm_password'])) {
                    $admin_user->password = md5(md5($data['password']));
                }
